...
object SampleStringsGenerator {
    ...
    fun getStrings(locale: Locale): Map<String, CharSequence> {
        ...
        when (locale) {
            ...
            LOCALE_PARAGUAY_SPANISH -> {
                map["title"] = "Titulo (desde restring)."
                map["subtitle"] = "Subtitulo (desde restring)."
                map["a_string_not_in_strings_xml"] = "Un recurso string que no se encuentra en strings.xml  $locale"
            }
        }
        return map
    }
